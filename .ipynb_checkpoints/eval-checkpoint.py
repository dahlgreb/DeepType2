...
def loss_supervised(alpha=0):
    def custom_loss(labels_true,logits_est):
        indexes = labels_true[:,5]
        labels = tf.reshape(1-tf.reduce_sum(labels_true[:,:5],1),(labels_true.shape[0],1))
        labels_true = tf.concat([labels_true[:,:5], labels], 1)
        loss_fun = tf.keras.losses.CategoricalCrossentropy()
        value = loss_fun(logits_est, labels_true)
        return value
    return custom_loss
