...
def supervised_1view(data, FLAGS, do_val = True):
    ...
    data.train.labels[:,-1] = list(range(data.train.labels.shape[0]))
    #import pandas as pd
    #labels_df = pd.DataFrame(data.train.labels)
    #labels_df[6] = labels_df.index
    #data.train.labels = labels_df.values
