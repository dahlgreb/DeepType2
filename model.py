...
class Autoencoder(Model):
    def __init__(self, shape):
        ...
        self.encoder = tf.keras.Sequential([
             layers.Flatten(),
            layers.Dense(shape[1][0], activation='sigmoid')
        ])
        self.decoder = tf.keras.Sequential([
              layers.Dense(shape[1][1], activation='sigmoid'),
              layers.Dense(shape[2], activation='sigmoid')
        ])
    ...
    def _setup_variables(self, initialize = False, initialize_data = []):
        for i in range(0, self.__num_hidden_layers + 1):
            ...
            if initialize:
                ...
                w_init = tf.Variable(tf.random.normal(w_shape, stddev=0.02))
            ...
            if initialize:
                ...
                b_init = tf.Variable(tf.random.normal(b_shape,  stddev=0.02))
